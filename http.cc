...
int http_read_status_line(char *buf, int buflen, char **line, size_t *line_length)
{
    size_t count = 0;
...
struct http_challenge *http_header_get_challenge(const struct http_header *header, struct http_challenge *challenge)
{
    const struct http_header *p;
    ...
    p = NULL;
    while ((p = http_header_next(header, p, "WWW-Authenticate")) != NULL) {
        const char *tmp;
