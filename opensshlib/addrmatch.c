...
addr_match_list(const char *addr, const char *_list)
{
	...
	while ((cp = strsep(&list, ",")) != NULL) {
		...
		if (r == -2) {
			ssh_error("Inconsistent mask length for "
			    "network \"%.100s\"", cp);
			ret = -2;
			break;
		} else if (r == 0) {
...
addr_match_cidr_list(const char *addr, const char *_list)
{
	...
	while ((cp = strsep(&list, ",")) != NULL) {
		if (*cp == '\0') {
			ssh_error("%s: empty entry in list \"%.100s\"",
			    __func__, o);
			ret = -1;
			break;
		}
		...
		if (strlen(cp) > INET6_ADDRSTRLEN + 3) {
			ssh_error("%s: list entry \"%.100s\" too long",
			    __func__, cp);
			ret = -1;
			break;
		}
#define VALID_CIDR_CHARS "0123456789abcdefABCDEF.:/"
		if (strspn(cp, VALID_CIDR_CHARS) != strlen(cp)) {
			ssh_error("%s: list entry \"%.100s\" contains invalid "
		...
		r = addr_pton_cidr(cp, &match_addr, &masklen);
		if (r == -1) {
			ssh_error("Invalid network entry \"%.100s\"", cp);
			ret = -1;
			break;
		} else if (r == -2) {
			ssh_error("Inconsistent mask length for "
